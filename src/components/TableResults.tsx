  ...
  if (!peopleExist) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center p-1.5">
        ...
        <h2 className="mb-1 text-base font-medium text-zinc-900 dark:text-white">
          No people added
        </h2>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-zinc-700">
              <thead>
                <tr className="divide-x divide-gray-200 dark:divide-zinc-700 dark:bg-zinc-900">
                  <th
                    scope="col"
                    className="w-3/5 px-6 py-3 text-left text-xs font-medium capitalize text-zinc-400"
                  >
                    Name
                  </th>
                  <th
                    scope="col"
                    className="w-2/5 px-6 py-3 text-left text-xs font-medium capitalize text-zinc-400"
                  >
                    Pay
                  </th>
                </tr>
...
const TotalPay = () => {
  ...
  const totalPriceMemo = useMemo(() => {
    return (
      values?.cut?.reduce(
        (acc, item) => acc + (item?.people?.length ? item?.price || 0 : 0),
        0
      ) || 0
    );
  }, [values]);
